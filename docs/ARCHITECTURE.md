...
Minis SSH Ops ships as one Android application with two cooperating parts:
...
The Android host starts the bundled Go executable as a local process. Flutter communicates with it over an authenticated HTTP service bound to `127.0.0.1`.
...
SSH and model secrets are not returned to Flutter after they are saved. Host responses expose only flags such as `hasPassword` and `hasPrivateKey`.

## Go Backend

The Go service owns sensitive data and remote operations.

- `backend/internal/api`: versioned HTTP handlers, authentication and WebSocket PTY
...
4. The backend executes the operation and records relevant audit data.
...
4. Required confirmation is enforced by the backend, independent of model output.
...
The current implementation stores the master key in the application data directory. Moving key protection into Android Keystore is a future hardening task.
...
GitHub Actions runs Go tests, a Linux backend smoke test, Flutter analysis/tests and an arm64 APK build. Release signing runs only when all required signing Secrets are configured.
