...
class ProbeSummary {
  ...
  factory ProbeSummary.fromProbeJson(Map<String, dynamic> res) {
    ...
    final lines = <ProbeLine>[
      ...
      ProbeLine('Memory%', memHint),
      if (memSub.isNotEmpty) ProbeLine('Memory', memSub),
      ProbeLine('Disk%', diskHint),
      if (diskSub.isNotEmpty) ProbeLine('Disk', diskSub),
      ProbeLine('Uptime', upHint),
      ProbeLine('System', sys),
      ProbeLine('Load', loadHint),
      ProbeLine('Details', detail.toString()),
    ];
